...
export class OpenVRService {
  private _status: BehaviorSubject<OpenVRStatus> =
    new BehaviorSubject<OpenVRStatus>('INACTIVE');
  public status: Observable<OpenVRStatus> = this._status.asObservable();
  private _devices: BehaviorSubject<OVRDevice[]> = new BehaviorSubject<
    OVRDevice[]
  >([]);
  ...
  onDeviceUpdate(device: OVRDevice) {
    if (device.class === 'Invalid') return;
    device = Object.assign({}, device);
    if (device.isTurningOff === null || device.isTurningOff === undefined)
      device.isTurningOff =
        this._devices.value.find((d) => d.index === device.index)
          ?.isTurningOff ?? false;
    ...
    this._devices.next(
      orderBy(
        [
          device,
          ...this._devices.value.filter((d) => d.index !== device.index),
        ],
        ['deviceIndex'],
        ['asc']
      )
