...
export function triggerChildren(name = 'triggerChildren', childQuery = '@*') {
  return trigger(name, [
    transition('* => *', [
      group([query(childQuery, [animateChild()], { optional: true })]),
    ]),
  ]);
}
...
export function fadeUp(name = 'fadeUp', length = '.15s ease') {
  return trigger(name, [
    ...
    transition(':leave', [
      animate(length, style({ opacity: 0, transform: 'translateY(44px)' })),
    ]),
  ]);
}
...
export function fadeLeft(name = 'fadeLeft', length = '.15s ease') {
  return trigger(name, [
    ...
    transition(':leave', [
      animate(length, style({ opacity: 0, transform: 'translateX(-44px)' })),
    ]),
  ]);
}
